...
public abstract class GameObject implements ImageTile {
    ...
    @Override public abstract int getLayer();
    public abstract int mutation();
    public abstract void move(Vector2D delta);

    /**
     * Factory: delega ao enumerado ObjectType.
     * Room cria sempre a Water no fundo; a fábrica devolve null para ' '.
     */
    public static GameObject fromChar(char c, Room r, int x, int y) {
        Point2D pos = new Point2D(x, y);
        ObjectType t = ObjectType.fromChar(c);
        if (t == ObjectType.UNKNOWN || t == ObjectType.WATER) return null;
