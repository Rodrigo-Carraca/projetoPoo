...
/**
 * Representa uma sala (Room).
 * Implementação de readRoom usando Scanner e fornece getObjects() público.
 */
public class Room {
            ...
            while (sc.hasNextLine()) {
                ...
                for (int x = 0; x < line.length(); x++) {
                    ...
                    if (go != null) {
                        ...
                        if (go instanceof BigFish) room.bigFish = (BigFish) go;
                        if (go instanceof SmallFish) room.smallFish = (SmallFish) go;
                    }
                }
    ...
    /**
     * Retorna a lista ordenada por layer, y e x — método público usado pelo GameEngine.
     */
    public List<ImageTile> getObjects() {
        ...
        Collections.sort(list, new Comparator<ImageTile>() {
            ...
            public int compare(ImageTile a, ImageTile b) {
                ...
                if (pa.getY() != pb.getY()) return Integer.compare(pa.getY(), pb.getY());
    ...
    public void setBigFish(BigFish b) { this.bigFish = b; }
    public void setSmallFish(SmallFish s) { this.smallFish = s; }

    public String getName() { return name; }


/**
 * Retorna o GameObject de mais alta layer na posição indicada (x,y),
 * ou null se não houver objecto nessa posição.
 */
public GameObject getTopObjectAt(pt.iscte.poo.utils.Point2D p) {
    GameObject top = null;
    for (GameObject go : objects) {
        if (go.getPosition() != null && go.getPosition().getX() == p.getX() && go.getPosition().getY() == p.getY()) {
...
/**
 * Verifica se a posição está dentro dos limites visíveis (delegate para ImageGUI).
 * Se ImageGUI não estiver inicializada, assume true para evitar NPEs.
 */
